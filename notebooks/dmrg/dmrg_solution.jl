...
function dmrg(ψ::MPS, mpo; maxsweeps = 20, tol = 1e-10, verbose = true)
    ...
    # pre-build every right environment, from the right boundary inward
    Renv = Vector{Any}(undef, N + 1)
    Renv[N+1] = right_boundary()
    for i in N:-1:1
        Renv[i] = grow_right(Renv[i+1], ψ.A[i], mpo[i])
    end

    # left environments grow as the centre moves; only Lenv[1] is known up front
    Lenv = Vector{Any}(undef, N + 1)
    Lenv[1] = left_boundary()
    ...
    for sweep in 1:maxsweeps
        ...
        # stop once the energy stops moving
...
single-site code. The three functions below are the worked solution to the two-site hands-on exercise.
"""
...
function apply_Heff2(Θ, L, W1, W2, R)
    # contract L, W1, W2, R onto the ket legs of Θ, leaving its two bra legs (t1, t2) open
...
function two_site_update(L, A1, A2, W1, W2, R; trunc, dir)
    # merge the two neighbours over their shared bond into one block
    @tensor Θ[a s1 s2; b] := A1[a s1; m] * A2[m s2; b]
    vals, vecs, _ = eigsolve(x -> apply_Heff2(x, L, W1, W2, R), Θ, 1, :SR; ishermitian = true)
    Θopt = vecs[1]
    e = real(vals[1])
    # regroup as [a s1; s2 b] so the SVD splits the block across the middle bond.
    # in the mixed-canonical gauge these singular values are the Schmidt values,
    # so truncating them here is globally optimal at this bond
    @tensor Θmat[a s1; s2 b] := Θopt[a s1 s2; b]
    U, S, Vh, ε = svd_trunc(Θmat; trunc)
    if dir === :right
        # A1 stays left-orthonormal; fold S into A2 to carry the centre right
        A1n = U
        @tensor A2n[a s; b] := S[a; m] * Vh[m; s b]
    else
        # A2 stays right-orthonormal; fold S into A1 to carry the centre left
        @tensor A1n[a s; b] := U[a s; m] * S[m; b]
        # regroup Vh from [c; phys right] into [c phys; right]
        @tensor A2n[c s; b] := Vh[c; s b]
    end
    return A1n, A2n, e, ε
end
...
function dmrg2(ψ::MPS, mpo; D = 32, cutoff = 1e-12, maxsweeps = 20, tol = 1e-10, verbose = true)
    N = length(ψ)
    right_canonicalize!(ψ)
    # cap the new bond at D, and drop any singular value below the cutoff
    ...
    for sweep in 1:maxsweeps
        maxε = 0.0
        # left → right: optimize each bond, then absorb the left tensor into Lenv
        for i in 1:N-1
        ...
        # right → left: the mirror image, absorbing the right tensor into Renv
        for i in N-1:-1:1
            A1, A2, e, ε = two_site_update(Lenv[i], ψ.A[i], ψ.A[i+1],
